...
(function () {
  ...
  var svg = d3.select("#chartCont").append("svg").attr("width", width).attr("height", height).append("g")
  ...
  function showErc() {
    var ercData = d3.keys(ercValuesX);
    var ercValues = svg.selectAll('.erc20').data(ercData);

    ercValues.enter().append('text').attr('class', 'erc20').attr('x', function (d) {
      return ercValuesX[d];
    }).attr('y', 200).attr('text-anchor', 'middle').text(function (d) {
      return d;
    });
  ...
  var simulation = d3.forceSimulation().force("xAxis", d3.forceX(width / 2).strength(0.03)).force("yAxis", d3.forceY(height / 2).strength(0.03)).force("preventCollide", d3.forceCollide(function (d) {
  ...
  function loaded(error, data) {
    ...
    var circle = svg.selectAll("circle").data(data).enter().append("circle").attr("class", "token")
